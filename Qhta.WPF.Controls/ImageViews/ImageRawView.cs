...
{
  public class ImageRawView : UIElement
  {
    public ImageRawView()
    {
    ...
    public static readonly DependencyProperty SourceProperty = DependencyProperty.Register(
        ...
        new FrameworkPropertyMetadata(
            ...
            new PropertyChangedCallback(ImageRawView.OnSourceChanged)));
    ...
    protected override Size MeasureCore(Size availableSize)
    {
      ...
      if (bitmapSource != null)
      {
        ...
        if (ps != null)
        {
          ...
          measureSizeV.X *= RenderScale.X;
          measureSizeV.Y *= RenderScale.Y;
          measureSize = new Size(measureSizeV.X, measureSizeV.Y);
        }
      }
      Debug.WriteLine($"Measure size = ({measureSize.Width}, {measureSize.Height})");
      return measureSize;
    }
    ...
    {
      ImageRawView targetObj = (ImageRawView)d;

      BitmapSource oldValue = (BitmapSource)e.OldValue;
      BitmapSource newValue = (BitmapSource)e.NewValue;

      if (((oldValue != null) && (targetObj._sourceDownloaded != null)) && (!oldValue.IsFrozen && (oldValue is BitmapSource)))
      {
        ((BitmapSource)oldValue).DownloadCompleted -= targetObj._sourceDownloaded;
        ((BitmapSource)oldValue).DownloadFailed -= targetObj._sourceFailed;
      }
      if (((newValue != null) && (newValue is BitmapSource)) && !newValue.IsFrozen)
      {
        ((BitmapSource)newValue).DownloadCompleted += targetObj._sourceDownloaded;
        ((BitmapSource)newValue).DownloadFailed += targetObj._sourceFailed;
      }
    }
    ...
    public static DependencyProperty RenderOffsetProperty = DependencyProperty.Register
      ("RenderOffset", typeof(Point), typeof(ImageRawView), new PropertyMetadata(new Point(0,0)));

    public Point RenderOffset
    {
      get
      {
        return (Point)GetValue(RenderOffsetProperty);
      }
      set
      {
    ...
    public static DependencyProperty RenderScaleProperty = DependencyProperty.Register
      ("RenderScale", typeof(Point), typeof(ImageRawView), new PropertyMetadata(new Point(1, 1)));

    public Point RenderScale
    {
      get
      {
        return (Point)GetValue(RenderScaleProperty);
      }
      set
      {
        SetValue(RenderScaleProperty, value);
      }
    }
    ...
    private Point GetPixelOffset()
    {
      ...
      if (ps != null)
      {
        ...
        pixelOffset.X += RenderOffset.X;
        pixelOffset.Y += RenderOffset.Y;
