...
public static class ObjectComparer
{
  ...
  private static CompareResult AreEqual(object? @this, object? other, string propName)
  {
    ...
    if (@this != null && other != null && @this.GetType() != other.GetType()) return new CompareResult { AreEqual = false, DiffPath = "@Type" };
    ...
    if (props != null)
      foreach (var propInfo in props)
      {
            ...
            && other is ICollection otherCollection)
        {
          ...
          if (thisCount != otherCount) return new CompareResult { AreEqual = false, DiffPath = "Count" };
  ...
  private static Task<CompareResult> AreEqualAsync(object? @this, object? other, string propName)
  {
    return Task.Factory.StartNew(() =>
    {
      ...
      if (@this?.GetType() != other?.GetType()) return new CompareResult { AreEqual = false, DiffPath = "@Type" };
      ...
      if (props != null)
        foreach (var propInfo in props)
        {
              ...
              && other is ICollection otherCollection)
          {
            ...
            otherIterator.Reset();
            var itemNumber = 0;
            var itemCompTasksList = new List<Task<CompareResult>>(props.Count());
            while (thisIterator.MoveNext())
            {
              otherIterator.MoveNext();
              var value1 = thisIterator.Current;
