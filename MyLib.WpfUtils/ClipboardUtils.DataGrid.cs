...
namespace MyLib.WpfUtils
{
  ...
  {
    public static bool CopyToClipboard(DataGrid element)
    {
      bool success = true;
      Clipboard.Clear();
      var html = MakeHtml(element, out string plainText);
          ...
          foreach (var obj in dataSet)
          {
            ...
            if (isSelected)
            {
              ...
              foreach (var column in columns)
              {
                ...
                sb.Append("<td>");
                sb.Append(HtmlTextUtils.EncodeHtmlEntities(txt== "" ? "\u00A0" : txt));
                sb.Append("</td>");
    ...
    //private static Binding GetBinding (this DataGridColumn column)
    //{
    //  if (column is DataGridTextColumn textColumn)
    //    return textColumn.Binding as Binding;
