...
public partial class CollectionViewBehavior
{
  ...
  #region EnableCompoundFilter property
  /// <summary>
  /// Getter of EnableCompoundFilter property.
  /// </summary>
  /// <param name="target"></param>
  /// <returns></returns>
  public static bool? GetEnableCompoundFilter(DependencyObject? target)
  {
    if (target == null) return null;
    if (target is DataGridColumnHeader header)
      return (bool)header.GetValue(EnableCompoundFilterProperty);
    var result = target.GetValue(EnableCompoundFilterProperty);
    return (bool?)result;
  }

  /// <summary>
  /// Setter of EnableCompoundFilter property.
  /// </summary>
  /// <param name="target"></param>
  /// <param name="value"></param>
  public static void SetEnableCompoundFilter(DependencyObject target, bool value)
  {
    target.SetValue(EnableCompoundFilterProperty, value);
  }
  /// <summary>
  /// Specifies whether a "Filter" button should be displayed in the column header.
  /// </summary>
  public static readonly DependencyProperty EnableCompoundFilterProperty = DependencyProperty.RegisterAttached(
      "EnableCompoundFilter",
      typeof(bool?),
      typeof(CollectionViewBehavior),
      new PropertyMetadata(null));
  #endregion
            ...
            if (column != null)
            {
              if (column.TryGetFilterablePropertyPath(itemType, out var propPath))
              {
                ...
                if (propertyInfo != null)
                {
                  ...
                  if (ok)
                  {
                    var columnFilterViewModel = GetColumnFilter(column) as FilterViewModel;
                    if (columnFilterViewModel != null)
                    {
                      var filter = columnFilterViewModel.CreateFilter();
                    ...
                    {
                      var filter = GetCollectionFilter(itemsControl) as IFilter;
                      if (sourceCollectionView != null)
                        sourceCollectionView.Filter = filter?.GetPredicate();
                    }
                    args.Handled = true;
                  }
                }
  ...
  public virtual bool DisplayFilterDialog(ItemsControl itemsControl, DataGridColumn column, PropPath propPath, Point position, Window? ownerWindow)
  {
    if (itemsControl is DataGrid dataGrid)
    {
      ...
      var origViewModel = (GetCollectionFilter(itemsControl) as DataGridFilterViewModel);
      var viewModel = origViewModel?.CreateCopy() as DataGridFilterViewModel;
      if (viewModel == null)
        viewModel = new DataGridFilterViewModel(dataGrid);
      ...
      viewModel.Column = viewModel.Columns?.FirstOrDefault(item => item.Column == column);
      ...
      {
        SetCollectionFilter(itemsControl, viewModel.CreateFilter());
        return true;
      }
    }
    return false;
